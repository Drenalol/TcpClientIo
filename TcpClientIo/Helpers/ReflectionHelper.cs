...
namespace Drenalol.Helpers
{
    internal class ReflectionHelper<TRequest, TResponse>
    {
        ...
        private static void EnsureTypeHasRequiredAttributes(Type type, ImmutableDictionary<int, TcpProperty> properties)
        {
            var key = properties.Where(item => item.Value.Attribute.TcpDataType == TcpDataType.Id).ToList();

            if (key.Count == 0)
                throw TcpException.Throw(TcpTypeException.AttributeKeyRequired, type.ToString());
            if (key.Count > 1)
